...
@Component({
  selector: 'app-duplicate-ads',
  templateUrl: './duplicate-ads.component.html',
  styleUrl: './duplicate-ads.component.css',
})
export class DuplicateAdsComponent implements OnInit {
  ad: any = {};
  categories: Category[] = [];
  selectedOption: any = {
    active: false,
  ...
  uploadFilesAndUpdateAnnonce(accessToken: string): void {
    ...
    Promise.all(
      this.selectedFiles.map((file) =>
        this.annonceService.uploadImage(file, accessToken).then((response) => {
          mediaIds.push(response.data.id);
        })
      )
    )
      .then(() => {
        const annonceId = { ad_id: this.adId };
        this.annonceService
          .duplicateAnnonce(annonceId, accessToken)
          .subscribe((rep) => {
                  ...
                  this.annonceService
                    ...
                    .subscribe(
                      ...
                      (error) => {
                        ...
                        if (
                          error.error.message === "This AdBook doesn't exist"
                        ) {
